...
(function($) {
    ...
    var crontasksmanagerproConfig = window.crontasksmanagerproConfig || {
        baseUrl: '',
        token: ''
    };
    ...
    function wrapTable() {
        ...
        if ($table.length && !$table.parent().hasClass('crontasksmanagerpro-table-wrapper')) {
            $table.wrap('<div class="crontasksmanagerpro-table-wrapper"></div>');
        }
    }
            ...
            if ($actions.length) {
                ...
                $actions.find('a').each(function() {
                    ...
                    if (href.indexOf('update') !== -1 || href.indexOf('edit') !== -1 || 
                        href.indexOf('addcrontasksmanagerpro') !== -1 || 
                ...
                if (!deleteLink) {
                    var deleteMatches = [
                        ...
                        originalActionsHtml.match(/href=["']([^"']*deletecrontasksmanagerpro[^"']*)["']/i),
                        originalActionsHtml.match(/deletecrontasksmanagerpro[&?]id_crontasksmanagerpro=(\d+)/i)
                    ];
                ...
                var idMatch = originalActionsHtml.match(/id_crontasksmanagerpro[=:](\d+)/i) || 
                             $row.find('input[type="checkbox"][name*="crontasksmanagerpro"]').val() ||
                             originalActionsHtml.match(/\bid_crontasksmanagerpro=(\d+)/i) ||
                ...
                if (!id) {
                    ...
                    var idFromRow = rowText.match(/id_crontasksmanagerpro[=:]?(\d+)/i);
                    if (idFromRow) {
                        id = idFromRow[1];
                    }
                }
                ...
                if (id) {
                    var baseUrl = crontasksmanagerproConfig.baseUrl || '';
                    var token = crontasksmanagerproConfig.token || '';
                    
                    if (!baseUrl || baseUrl === '') {
                        baseUrl = window.location.href.split('?')[0] + '?controller=AdminCronTasksManagerPro';
                    }
                    
                    if (!editLink) {
                        editLink = baseUrl + '&id_crontasksmanagerpro=' + id + '&updatecrontasksmanagerpro&token=' + token;
                    }
                    // ALWAYS build deleteLink if we have ID
                    if (!deleteLink || deleteLink === '#') {
                        deleteLink = baseUrl + '&id_crontasksmanagerpro=' + id + '&deletecrontasksmanagerpro&token=' + token;
                    }
                    if (!executeLink) {
                        executeLink = baseUrl + '&id_crontasksmanagerpro=' + id + '&executeNowcrontasksmanagerpro&token=' + token;
                    }
                }
                
                // Build new buttons with Bootstrap 5
                if (editLink || deleteLink || executeLink) {
                    var newActions = '<div class="crontasksmanagerpro-actions">';
